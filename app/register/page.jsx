    ...
    try {
      ...
      if (error) {
        ...
        if (error.message?.toLowerCase().includes("already registered")) {
          setMsg(
            "هذا البريد مسجل بالفعل. سجل الدخول بدلًا من إنشاء حساب جديد.",
          );
        } else {
          setMsg(
            `تعذر إنشاء الحساب: ${error.message || "يرجى مراجعة البيانات والمحاولة مجددًا."}`,
          );
        }
        return;
      }
      ...
      if (data.session) {
        setMsg(
          "تم إنشاء الحساب وتسجيل الدخول بنجاح. جاري تثبيت الجلسة وتحويلك...",
        );
        await redirectWithVerifiedSession(supabase, data.session, redirect);
      } else {
        setMsg(
          "تم إنشاء الحساب. افتح رسالة Supabase في بريدك واضغط تأكيد البريد، ثم سيتم تسجيل دخولك.",
        );
      }
    } catch (err) {
      setIsSuccess(false);
      setMsg(
        err.message || "حدث خطأ غير متوقع أثناء إنشاء الحساب. حاول مرة أخرى.",
      );
    } finally {
      setIsLoading(false);
    }
        ...
        >
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div>
              ...
              >
                {isLoading
                  ? "جاري تسجيل التصاريح والمقرات..."
                  : "إنشاء العضوية وبدء المعركة"}
              </button>
            </div>
          </form>
